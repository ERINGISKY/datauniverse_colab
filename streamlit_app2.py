...
# モデルとトークナイザーのオンデマンドロードを行う関数
def load_bert_model():
    #tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-uncased') # メモリ使用量が少ない軽量モデル
    #model = BertModel.from_pretrained('bert-base-uncased')
    model = DistilBertModel.from_pretrained('distilbert-base-uncased') # メモリ使用量が少ない軽量モデル
    return tokenizer, model

# 修正後のベクトル化関数
def get_bert_embedding(text):
    tokenizer, model = load_bert_model()  # この行を追加
